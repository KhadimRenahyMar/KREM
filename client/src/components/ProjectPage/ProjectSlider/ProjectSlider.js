...
export default function ProjectSlider({ project }) {
    ...
    useEffect(() => {
        ...
        if (project) {
            const localShots = JSON.parse(localStorage.getItem(`${project.title} screenshots`));
            console.log('localshots =', localShots);
            if (localShots === null) {
                const fetchScreenshotsGithubAPI = async () => {
                    console.log("coucou le fetch");
                    const data = await axios.get('https://api.github.com/repos/KhadimRenahyMar/Arch-iQ/contents/portfolio/screenshots')
                    // console.log(data.data);
                    fetchedPNG.push(data.data);
                    setScreenshots(...fetchedPNG);
                    localStorage.setItem(`${project.title} screenshots`, JSON.stringify(...fetchedPNG));
                }
                fetchScreenshotsGithubAPI();
            }
            else{
